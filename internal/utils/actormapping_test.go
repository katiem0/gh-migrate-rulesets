...
func writeActorMappingCSV(t *testing.T, content string) string {
	t.Helper()
	path := filepath.Join(t.TempDir(), "actors.csv")
...
func TestLoadActorMapping(t *testing.T) {
	...
	t.Run("parses rows and ignores extra columns", func(t *testing.T) {
		path := writeActorMappingCSV(t, "actor_type,source_id,source_name,target_id\nRepositoryRole,4,Write,9\nTeam,100,platform,200\n")
	...
	t.Run("actor_type is matched case-insensitively", func(t *testing.T) {
		path := writeActorMappingCSV(t, "actor_type,source_id,target_id\nrepositoryrole,4,9\n")
		got, err := LoadActorMapping(path)
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
	...
	t.Run("missing headers errors", func(t *testing.T) {
		path := writeActorMappingCSV(t, "actor_type,source_id\nRepositoryRole,4\n")
	...
	t.Run("missing file errors", func(t *testing.T) {
		if _, err := LoadActorMapping(filepath.Join(t.TempDir(), "missing.csv")); err == nil {
...
func TestResolveMappedActorID(t *testing.T) {
	mapping := map[string]int{actorMappingKey("RepositoryRole", 5): 42}

	if id, ok := resolveMappedActorID(mapping, "RepositoryRole", 5); !ok || id != 42 {
		t.Errorf("resolveMappedActorID() = (%d, %v), want (42, true)", id, ok)
	}
	if _, ok := resolveMappedActorID(mapping, "RepositoryRole", 6); ok {
		t.Error("resolveMappedActorID() found unexpected mapping")
	}
	if _, ok := resolveMappedActorID(nil, "RepositoryRole", 5); ok {
		t.Error("resolveMappedActorID() should return false for nil mapping")
	}
}
